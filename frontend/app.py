...
with st.sidebar:
    ...
    if method == "enter_text":
        ...
        if uploaded:
            ...
            try:
                ...
                file_content = raw.decode("cp1252", errors="ignore")
    ...
    def do_simplify():
        payload = text_input if method == "enter_text" else file_content
        if not payload.strip():
            ...
            with st.spinner(tlabel("Simplifying…")):
                try:
                    ...
                    st.session_state.prev_language = st.session_state["ui_language_name"]
...
with col2:
    ...
    def _on_lang_change():
        st.session_state["ui_language_code"] = lang_map[st.session_state["ui_language_name"]]        
        st.session_state["run_summary"] = False
        do_simplify()
    st.selectbox(label=tlabel("Choose Language:"), options=names, index=idx, key="ui_language_name", 
        on_change=_on_lang_change, label_visibility="collapsed")
        ...
        if st.button(tlabel("Send"), key="send_chat"):
            if not user_q.strip():
                ...
                with st.spinner(tlabel("Contacting assistant…")):
                    try:
                        res = requests.post(CHAT_URL, json={"user_id":"user1","user_message":user_q, "concise_summary": st.session_state["concise_summary"], "sections": st.session_state["sections"]})
                        res.raise_for_status()
