...
def convert_to_composition(data: dict, patient_id: str, author_reference: Optional[str] = None) -> Composition:
    ...
    to_take  = meds.get("ToTake", [])
    to_avoid = meds.get("ToAvoid", [])
    if to_take or to_avoid:
        parts: list[str] = []
        if to_take:
            parts.append("<strong>To Take:</strong><ul>")
            parts += [f"<li>{m}</li>" for m in to_take]
            parts.append("</ul>")
        if to_avoid:
            parts.append("<strong>To Avoid:</strong><ul>")
            parts += [f"<li>{m}</li>" for m in to_avoid]
            parts.append("</ul>")
        comp_dict["section"].append({
            "title": "Medications",
            "text": {
                "status": "generated",
                "div": "<div>" + "".join(parts) + "</div>"
            }
        })
